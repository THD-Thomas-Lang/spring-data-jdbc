...
/**
 * Used as an abstract class to derive relational writer actions (write, insert, update).
 * Implementations see {@link RelationalEntityWriter} and {@link RelationalEntityInsertWriter}
 *
 * @author Thomas Lang
 */
public abstract class AbstractRelationalEntityWriter implements EntityWriter<Object, AggregateChange<?>> {
    ...
    class WritingContext {
        ...
        private final Map<RelationalEntityInsertWriter.PathNode, DbAction> previousActions = new HashMap<>();
        private Map<PersistentPropertyPath<RelationalPersistentProperty>, List<RelationalEntityInsertWriter.PathNode>> nodesCache = new HashMap<>();
         ...
         */
        List<DbAction<?>> writeWithoutIsNewCheck() {
...
//        }
        ...
        private List<RelationalEntityInsertWriter.PathNode> from(PersistentPropertyPath<RelationalPersistentProperty> path) {
        ...
        private List<RelationalEntityInsertWriter.PathNode> createNodes(PersistentPropertyPath<RelationalPersistentProperty> path,
            ...
            if (path.getRequiredLeafProperty().isQualified()) {
                ...
                if (path.getRequiredLeafProperty().isMap()) {
                    ((Map<?, ?>) value).forEach((k, v) -> nodes.add(new RelationalEntityInsertWriter.PathNode(path, parentNode, Pair.of(k, v))));
                } else {
